...
local function CreateNoiseTexture()
	...
	for i = 1, size * size do
		local val = random(0, 255)
		data[i] = { val, val, val, 255 }
	end

	return glCreateTexture(size, size, {
		border = false,
...
local function GetActiveUnitInfo()
	...
	if weaponInfo.type == "ballistic" and weaponInfo.reloadWeaponHeightFrame and weaponInfo.reloadWeaponHeightFrame <= frame then
		local wx, wy, wz = spGetUnitWeaponVectors(unitId, weaponInfo.weaponNum)
		local ux, uy, uz = spGetUnitPosition(unitId)
		weaponInfo.weaponHeight = wy - uy
		local reloadFrame = spGetUnitWeaponState(unitId, weaponInfo.weaponNum, "reloadFrame")
...
-- CALLINS
--------------------------------------------------------------------------------
local function CreateShaderAndTexture()
